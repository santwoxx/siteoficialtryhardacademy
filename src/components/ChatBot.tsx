...
export const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Salve, futuro Pro-Player! 🎮 Sou o Tryhard Bot. Pronto para esmagar a concorrência e provar que seu QI é de elite? O que você precisa para dominar a arena?' }
  ]);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-24 md:bottom-6 right-6 z-[150] w-16 h-16 bg-neon-cyan text-black rounded-full shadow-[0_0_20px_rgba(0,255,255,0.4)] flex items-center justify-center hover:scale-110 transition-transform active:scale-95"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-44 md:bottom-24 right-6 z-[150] w-[calc(100vw-48px)] max-w-[380px] h-[550px] bg-[#121212] border border-white/10 rounded-[32px] shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="px-6 py-2 flex gap-2 overflow-x-auto scrollbar-hide">
              <button 
                onClick={() => setInput('Como entro para a Elite?')}
              ...
              <button 
                onClick={() => setInput('Quero o desconto de 68%')}
                className="whitespace-nowrap px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold text-gray-400 hover:bg-white/10 transition-colors"
              >
                🔥 Cupom 68%
              </button>
